...
public class PoissonUtil {
     ...
     */
    public static <T> List<Point<T>> generatePoints(
        int width,
        int height,
        int maxRetries,
        RandomSource random,
        @Nonnull List<PointConfiguration<T>> pointConfigurations
    ) {
    ...
    public static <T> List<Point<T>> generatePoints(
        ...
        RandomSource random,
        @Nonnull List<PointConfiguration<T>> pointConfigurations,
        @Nonnull List<Point<T>> initialPoints
    ) {
            ...
            retryLoop: for (int i = 0; i < maxRetries; i++) {
                ...
                Point<T> newPoint = new Point<>(
                    getRandomPositionAroundPoint(
                        ...
                        activePoint.config.radius,
                        activePoint.config.radius + largestRadius,
                        random
                    ),
                ...
                if (
                    newPoint.position.x < -widthBoundary ||
                    newPoint.position.x >= widthBoundary ||
                    newPoint.position.y < -heightBoundary ||
                    newPoint.position.y >= heightBoundary
                ) {
                    continue;
                }
    ...
    private static <T> PointConfiguration<T> getRandomPointConfiguration(
        RandomSource random,
        List<PointConfiguration<T>> items,
        double totalWeight
    ) {
