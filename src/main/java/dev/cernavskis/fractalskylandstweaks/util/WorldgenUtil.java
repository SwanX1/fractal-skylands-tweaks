...
public final class WorldgenUtil {
    ...
    private WorldgenUtil() {} // Uninstantiable
    ...
    public static boolean isChunkSpawn(ChunkPos chunk, LevelData level) {
        ...
        return (
            chunk.getMinBlockX() <= spawn.getX() &&
            spawn.getX() <= chunk.getMaxBlockX() &&
            chunk.getMinBlockZ() <= spawn.getZ() &&
            spawn.getZ() <= chunk.getMaxBlockZ()
        );
    }
    ...
    public static boolean isWithinChunk(ChunkPos chunk, BlockPos pos, int leeway) {
        return (
            chunk.getMinBlockX() - leeway <= pos.getX() &&
            pos.getX() <= chunk.getMaxBlockX() + leeway &&
            chunk.getMinBlockZ() - leeway <= pos.getZ() &&
            pos.getZ() <= chunk.getMaxBlockZ() + leeway
        );
    }
    ...
    public static boolean isWithinBoundary(ChunkPos chunk, int boundary) {
        ...
        return (
            chunk.getMinBlockX() >= -halfBoundary &&
            chunk.getMinBlockZ() >= -halfBoundary &&
            chunk.getMaxBlockX() <= halfBoundary &&
            chunk.getMaxBlockZ() <= halfBoundary
        );
    }
}
