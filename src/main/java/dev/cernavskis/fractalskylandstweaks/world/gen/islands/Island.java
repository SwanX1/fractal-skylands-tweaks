...
public class Island {
    ...
    public void generateTerrain(WorldGenLevel level, ChunkAccess chunkAccess, ChunkGenerator chunkGenerator) {
        ...
        double baseNoisePeak = Island.getNoisePeak(
            (x, z) -> this.getBaseNoise(baseSeed, x, z),
            (int) (iterableRadius / (this.config.terrainSettings.groundNoiseScale * 8))
        );
        double surfaceNoisePeak = Island.getNoisePeak(
            (x, z) -> this.getSurfaceNoise(surfaceSeed, x, z),
            (int) (iterableRadius / (this.config.terrainSettings.surfaceNoiseScale * 8))
        );
        ...
        for (int x = -(int) iterableRadius; x < iterableRadius; x++) {
            for (int z = -(int) iterableRadius; z < iterableRadius; z++) {
                ...
                double height =
                    (Math.abs(surfaceNoise - surfaceNoisePeak) + 1) *
                    (actualRadius - distance) *
                    this.config.terrainSettings.surfaceNoiseMultiplier;
                double depth =
                    (Math.abs(baseNoise - baseNoisePeak) + 1) *
                    (actualRadius - distance) *
                    this.config.terrainSettings.groundNoiseMultiplier;

                for (int y = -(int) depth; y <= height; y++) {
                    BlockState blockState =
                        this.config.blockStateSupplier.apply(height, depth, actualRadius, pos.relative(Axis.Y, y), baseSeed + 1);
                    if (blockState != null) {
    ...
    protected double getShapeNoise(long seed, double angle) {
        return (
            this.getAngledNoise(seed, angle, this.config.terrainSettings.shapeNoiseScale) * this.config.terrainSettings.shapeNoiseMultiplier
        );
    }

    protected double getSurfaceNoise(long seed, int x, int z) {
        return OpenSimplex2F.noise2(
            seed,
            x * this.config.terrainSettings.surfaceNoiseScale,
            z * this.config.terrainSettings.surfaceNoiseScale
        );
    }
    ...
    protected double getBaseNoise(long seed, int x, int z) {
        return OpenSimplex2F.noise2(
            seed,
            x * this.config.terrainSettings.groundNoiseScale,
            z * this.config.terrainSettings.groundNoiseScale
        );
    }
