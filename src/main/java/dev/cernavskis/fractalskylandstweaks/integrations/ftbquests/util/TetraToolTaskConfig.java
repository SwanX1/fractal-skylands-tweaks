...
public class TetraToolTaskConfig {
  ...
  public void fillConfigGroup(ConfigGroup config) {
    config.addEnum("modular_item", this.modularItemType, value -> this.modularItemType = value, NameMap.of(ModularItemType.ANY, ModularItemType.values()).create());
    ...
    for (ToolAction action : ToolAction.getActions()) {
      requiredToolLevelGroup.addInt(action.name(), this.requiredToolLevel.getOrDefault(action, 0), value -> this.requiredToolLevel.put(action, value), this.requiredToolLevel.getOrDefault(action, 0), 0, Integer.MAX_VALUE);
    }
  }
